...
intervalo_interquartil <- IQR(subset_salarios$REMUNERACAO_REAIS)
desvio_padrao <- sd(subset_salarios$REMUNERACAO_REAIS)

razao <- intervalo_interquartil/desvio_padrao
razao
```
...
calc_desv_abs_mediana <- function(x) {
  (dam_data <- median( abs( x - median( x ))))

  (md_data <- median( x ))

  retorno <- dam_data / md_data
  
  return(retorno)
}

ano_data_diploma <- year(subset_salarios$DATA_DIPLOMA_INGRESSO_SERVICOPUBLICO)

subset_salarios %>%
  summarise(intervalo_interquartil <- IQR(ano_data_diploma),
            desvio_padrao <- sd(ano_data_diploma),
            desvio_absoluto <- calc_desv_abs_mediana(ano_data_diploma)
  )
...
correlacao <- cor(x = year(subset_salarios$DATA_INGRESSO_ORGAO), y = year(subset_salarios$DATA_DIPLOMA_INGRESSO_SERVICOPUBLICO))
